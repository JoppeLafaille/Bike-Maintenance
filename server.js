...
const ACTIVITIES_FILE = path.join(__dirname, "strava-activities.json");
...
function readJsonFile(file, fallback) {
    try {
        ...
        console.error("Fout bij lezen van bestand:", file);
        console.error(error);
        return fallback;
    }
}
...
async function getValidAccessToken() {
    ...
    console.log("Strava access token verlopen. Vernieuwen...");
    ...
    if (!response.ok) {
        console.error("Strava token refresh fout:", data);
        return null;
    }
    ...
    saveTokenData(updatedTokenData);

    console.log("Strava access token vernieuwd.");
...
app.get("/api/strava/auth", (req, res) => {
    if (!isStravaConfigured()) {
        return res.status(500).send(
            "Strava is nog niet ingesteld in .env."
        );
    }
...
app.get("/api/strava/callback", async (req, res) => {
    ...
    if (error) {
        return res.send(`
            ...
            <body style="font-family: Arial; padding: 40px;">
                <h1>Strava-koppeling geannuleerd</h1>
                <p>Je hebt de koppeling geannuleerd.</p>
                <a href="/">Terug naar Bike Maintenance</a>
            </body>
            </html>
        `);
    }
    ...
    try {
        ...
        if (!response.ok) {
            console.error("Strava OAuth fout:", data);
            ...
            return res.status(500).send(`
                ...
                <body style="font-family: Arial; padding: 40px;">
                    <h1>Strava-koppeling mislukt</h1>
                    <p>Strava kon de koppeling niet voltooien.</p>
                    <a href="/">Terug naar Bike Maintenance</a>
                </body>
                </html>
            `);
        ...
        console.log("Strava succesvol gekoppeld.");
        ...
        res.send(`
            ...
            <body style="font-family: Arial; padding: 40px; text-align: center;">
                <h1>🚴 Strava succesvol gekoppeld!</h1>
                <p>Je wordt teruggestuurd naar Bike Maintenance.</p>
            </body>
            </html>
        `);

    } catch (error) {
        console.error("Strava callback fout:", error);

        res.status(500).send(`
            <h1>Er ging iets mis</h1>
            <p>Er kon geen verbinding met Strava worden gemaakt.</p>
            <a href="/">Terug naar Bike Maintenance</a>
        `);
    }
});
    ...
    try {
        ...
        if (!accessToken) {
            return res.status(401).json({
                error: "Strava is niet verbonden."
            });
        }
        ...
        if (!stored.initialized) {
            // Eerste keer:
            // alleen de laatste 7 dagen ophalen.
        ...
        stored.syncAfter = newestTimestamp;
...
app.get("/api/test", (req, res) => {
    res.json({
        status: "ok",
        message: "Bike Maintenance server werkt!"
    });
});
